...
module.exports = class ProductsDBApi {
  static async create(data, options) {
    ...
    await products.setMoreProducts(data.moreProducts || [], {
      transaction,
    });
  ...
  static async findBy(where, options) {
    ...
    output.moreProducts = await products.getMoreProducts({
      transaction,
    });
  ...
  static async findAll(filter, options) {
    ...
    let include = [
...

      {
        model: db.products,
        as: 'moreProducts',
        through: filter.moreProducts
          ? {
              where: {
                [Op.or]: filter.moreProducts.split('|').map((item) => {
                  return { ['Id']: Utils.uuid(item) };
                }),
              },
            }
          : null,
        required: filter.moreProducts ? true : null,
      },
